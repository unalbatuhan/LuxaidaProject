    ...
    if ( !class_exists( 'STAdminAPI' ) ) {
        class STAdminAPI extends STAdmin
        {
            ...
            function init()
            {
                ...
                add_action( 'wp_ajax_st_update_content_api_roomorama', [ $this, 'st_update_content_api_roomorama_func' ] );
                add_action( 'wp_ajax_nopriv_st_update_content_api_roomorama', [ $this, 'st_update_content_api_roomorama_func' ] );
            }
            ...
            function st_update_content_api_roomorama_func()
            {
                ...
                switch ( $st_step ) {
                    case "1":
                        ...
                        $data     = self::st_get_data( $url_link, [ 'access_token' => $access_token ] );
                    ...
                    case "2":
                        ...
                        $limit    = 1;
                        $data     = self::st_get_data( $url_link, [ 'access_token' => $access_token, 'page' => $page, 'limit' => $limit ] );
                        if ( !empty( $data->result ) ) {
            ...
            static function is_optiontree_page()
            {
                if ( is_admin()
                    and isset( $_GET[ 'page' ] )
                    and $_GET[ 'page' ] == 'st_traveler_option'
                ) return TRUE;
            ...
            {
                wp_enqueue_script( 'admin_api.js', get_template_directory_uri() . '/inc/js/admin_api.js', [ 'jquery' ], NULL, TRUE );
